// Lists of words that trigger the responses
const happyList = ["happy", "great", "good", "better", "joyful", "content", "fulfilled", "positive", "optimistic", "upbeat", "elated", "radiant", "grateful", "blissful", "satisfied", "cheerful", "exuberant", "vibrant", "ecstatic", "peaceful", "serene", "blessed", "delighted", "glorious", "heartened", "inspired", "triumphant", "zealous", "merry", "euphoric", "jovial", "gleeful", "festive", "lively", "spirited", "buoyant", "uplifting", "hopeful", "comforted", "empowered", "wonderful"];
const sadList = ["sad", "negative", "boring", "melancholy", "unhappy", "disheartened", "discouraged", "gloomy", "dismal", "despondent", "downcast", "mournful", "blue", "downhearted", "crestfallen", "dejected", "forlorn", "sullen", "somber", "lugubrious", "woeful", "heartbroken", "bereaved", "mournful", "pensive", "doleful", "woebegone", "disconsolate", "lugubrious", "wistful", "regretful", "downtrodden", "defeated", "low-spirited", "demoralized", "dispirited", "apathetic", "listless", "weary", "dreary", "stifled", "glum", "uninspired", "unmotivated", "unfulfilled"];
const angryList = ["angry", "irate", "enraged", "furious", "livid", "incensed", "outraged", "resentful", "indignant", "annoyed", "irritated", "agitated", "frustrated", "exasperated", "infuriated", "provoked", "cross", "mad", "upset", "offended", "vexed", "displeased", "hostile", "bitter", "resentful", "miffed", "peeved", "sulky", "testy", "tense", "grumpy", "irascible", "cranky", "impatient", "irate", "aggravated", "disgruntled", "exasperated", "hot-tempered", "short-tempered", "snappish", "touchy"];
const depressedList = ["depressed", "downhearted", "melancholic", "disheartened", "blue", "low", "despondent", "despairing", "dismal", "gloomy", "forlorn", "sorrowful", "mournful", "wretched", "hopeless", "discouraged", "downtrodden", "disconsolate", "downcast", "miserable", "heavy-hearted", "sad", "unhappy", "glum", "joyless", "lugubrious", "morose", "somber", "sullen", "woeful", "tearful", "weepy", "weeping", "sulky", "pessimistic", "defeated", "crestfallen", "brokenhearted", "dejected", "desolate", "heartbroken", "inconsolable", "morbid", "unconsolable"];
const suicidalList = ["suicidal", "hopeless", "desperate", "worthless", "overwhelmed", "lost", "trapped", "unbearable", "broken", "alone", "helpless", "painful", "endless", "darkness", "ending it", "ending my life", "cannot go on", "no way out", "giving up", "life is meaningless", "thinking of suicide", "suicide thoughts", "ending it all", "just want it to stop", "can't go on", "want to die", "don't want to live", "no reason to live", "wish I were dead", "I'm a burden", "tired of life", "nothing to live for", "fade away", "wish it would end", "permanent solution", "escape the pain", "end the suffering", "last resort"];

function getResponse(input) {
    input = input.toLowerCase();
    if (suicidalList.some(word => input.includes(word))) {
        return suicidalMoodResponses();
    } else if (depressedList.some(word => input.includes(word))) {
        return depressedMoodResponses();
    } else if (angryList.some(word => input.includes(word))) {
        return angryMoodResponses();
    } else if (sadList.some(word => input.includes(word))) {
        return sadMoodResponses();
    } else if (happyList.some(word => input.includes(word))) {
        return happyMoodResponses();
    } else {
        return generalResponses();
    }
}

function happyMoodResponses() {
    const responses = [
...
function depressedMoodResponses() {
    const responses = [
...
function suicidalMoodResponses() {
    const responses = [
...
function displayUserMessage(message) {
    ...
    newChat.classList.add('chat', 'outgoing');
    newChat.innerHTML = `
...
function displayResponse(response) {
    const chatContainer = document.getElementById('chat-container');
    const newChat = document.createElement('div');
    newChat.classList.add('chat', 'incoming');
    newChat.innerHTML = `
        <div class="chat-content">
            <div class="chat-details">
                <img src="assets/Eva.png" alt="friend-icon">
                <p>${response}</p>
            </div>
        </div>
    `;
